...
if [ $? = "1" ]; then
	...
	printf "files are NOT the same. copy config.shover to  ~/.config/tmux-powerline manually \n"
	printf "\ncp config.sh ~/.config/tmux-powerline/config.sh \n"
else
	printf "files are the same. copy config.sh over to ~/.config/tmux-powerline for you \n"
	cp config.sh ~/.config/tmux-powerline/config.sh
fi
